...
pace_model <- glm(ortg_next ~ ortg + ortg_l1 + ortg_l2 + 
                    pct_minutes_stay + 
...
pace_model_simple <- glm(ortg_next ~ ortg+ ortg_l1 + ortg_l2,
                         data = pace_data %>%
                           filter(season != test_season))
...
dpace_model <- glm(drtg_next ~ drtg + drtg_l1 + drtg_l2 + 
                     pct_minutes_stay + 
                     commits + avg_rtg_cmt + max_rtg_cmt +
                     transfers + avg_rtg_tfr + max_rtg_tfr,
                   data = pace_data %>%
...
dpace_model_simple <- glm(drtg_next ~ drtg + drtg_l1 + drtg_l2,
                          data = pace_data %>%
                            filter(season != test_season))
...
oreb_model <- glm(ortg_next ~ ortg + ortg_l1 + ortg_l2 + 
...
oreb_model_simple <- glm(ortg_next ~ ortg + ortg_l1 + ortg_l2,
                         data = oreb_data %>%
                           filter(season != test_season))


summary(oreb_model)
summary(oreb_model_simple)
...
dreb_model <- glm(drtg_next ~ drtg + drtg_l1 + drtg_l2 + 
...
dreb_model_simple <- glm(drtg_next ~ drtg + drtg_l1 + drtg_l2,
                         data = oreb_data %>%
                           filter(season != test_season))
...
oreb_data %>%
  filter(season == test_season) %>% 
  cbind(preds = predict(dreb_model, oreb_data %>% filter(season == test_season))) %>% 
  summarise(cor = cor(preds, drtg_next), rmse = Metrics::rmse(drtg_next, preds))

oreb_data %>%
  filter(season == test_season) %>% 
  cbind(preds = predict(dreb_model_simple, oreb_data %>% filter(season == test_season))) %>% 
  summarise(cor = cor(preds, drtg_next), rmse = Metrics::rmse(drtg_next, preds))
...
oto_model <- glm(ortg_next ~ ortg + ortg_l1 + ortg_l2 + 
...
oto_model_simple <- glm(ortg_next ~ ortg + ortg_l1 + ortg_l2,
                        data = to_data %>%
                          filter(season != test_season))
...
to_data %>%
  filter(season == test_season) %>% 
  cbind(preds = predict(oto_model, to_data %>% filter(season == test_season))) %>% 
  summarise(cor = cor(preds, ortg_next), rmse = Metrics::rmse(ortg_next, preds))

to_data %>%
  filter(season == test_season) %>% 
  cbind(preds = predict(oto_model_simple, to_data %>% filter(season == test_season))) %>% 
  summarise(cor = cor(preds, ortg_next), rmse = Metrics::rmse(ortg_next, preds))

### Create model
test_season <- 2022

dto_model <- glm(drtg_next ~ drtg + drtg_l1 + drtg_l2 + 
...
dto_model_simple <- glm(drtg_next ~ drtg + drtg_l1 + drtg_l2,
                        data = to_data %>%
                          filter(season != test_season))
...
to_data %>%
  filter(season == test_season) %>% 
  cbind(preds = predict(dto_model, to_data %>% filter(season == test_season))) %>% 
  summarise(cor = cor(preds, drtg_next), rmse = Metrics::rmse(drtg_next, preds))

to_data %>%
  filter(season == test_season) %>% 
  cbind(preds = predict(dto_model_simple, to_data %>% filter(season == test_season))) %>% 
  summarise(cor = cor(preds, drtg_next), rmse = Metrics::rmse(drtg_next, preds))
...
ast_model <- glm(ortg_next ~ ortg + ortg_l1 + ortg_l2 +
                   pct_minutes_stay + ast_stay + efg_stay +
                   commits + avg_rtg_cmt + max_rtg_cmt +
...
ast_model_simple <- glm(ortg_next ~ ortg + ortg_l1 + ortg_l2,
                        data = ast_data %>%
                          filter(season != test_season))
...
astd_model <- glm(drtg_next ~ drtg + drtg_l1 + drtg_l2 + 
                    pct_minutes_stay + 
                    commits + avg_rtg_cmt + max_rtg_cmt +
                    transfers + avg_rtg_tfr + max_rtg_tfr,
                  data = ast_data %>%
...
astd_model_simple <- glm(drtg_next ~ drtg + drtg_l1 + drtg_l2,
                         data = ast_data %>%
                           filter(season != test_season))
...
ast_data %>%
  filter(season == test_season) %>% 
  cbind(preds = predict(astd_model, ast_data %>% filter(season == test_season))) %>% 
  summarise(cor = cor(preds, drtg_next), rmse = Metrics::rmse(drtg_next, preds))

ast_data %>%
  filter(season == test_season) %>% 
  cbind(preds = predict(astd_model_simple, ast_data %>% filter(season == test_season))) %>% 
  summarise(cor = cor(preds, drtg_next), rmse = Metrics::rmse(drtg_next, preds))
...
rating_model <- glm(ortg_next ~ ortg + ortg_l1 + ortg_l2 +
...
rating_model_simple <- glm(ortg_next ~ ortg + ortg_l1 + ortg_l2,
                           data = rtg_data %>%
                             filter(season != test_season))
...
rtg_data %>%
  filter(season == test_season) %>% 
  cbind(preds = predict(rating_model, rtg_data %>% filter(season == test_season))) %>% 
  summarise(cor = cor(preds, ortg_next), rmse = Metrics::rmse(ortg_next, preds))

rtg_data %>%
  filter(season == test_season) %>% 
  cbind(preds = predict(rating_model_simple, rtg_data %>% filter(season == test_season))) %>% 
  summarise(cor = cor(preds, ortg_next), rmse = Metrics::rmse(ortg_next, preds))

### Create defensive rating model ----------------
test_season <- 2021

ratingd_model <- glm(drtg_next ~ drtg + drtg_l1 + drtg_l2 + 
...
ratingd_model_simple <- glm(drtg_next ~ drtg + drtg_l1 + drtg_l2,
                            data = rtg_data %>%
                              filter(season != test_season))
...
rtg_data %>%
  filter(season == test_season) %>% 
  cbind(preds = predict(ratingd_model, rtg_data %>% filter(season == test_season))) %>%
  summarise(cor = cor(preds, drtg_next), rmse = Metrics::rmse(drtg_next, preds))

rtg_data %>%
  filter(season == test_season) %>% 
  cbind(preds = predict(ratingd_model_simple, rtg_data %>% filter(season == test_season))) %>% 
  summarise(cor = cor(preds, drtg_next), rmse = Metrics::rmse(drtg_next, preds))
...
## Rating ----------------------------
### Offense -------------
ortg_model <- glm(ortg_next ~ ortg + ortg_l1 + ortg_l2 +
                    pct_minutes_stay + efg_stay + obpm_stay +
                    commits + avg_rtg_cmt + max_rtg_cmt +
                    transfers + avg_rtg_tfr + max_rtg_tfr +
                    efg + obpm,
...
### Defense --------
drtg_model <- glm(drtg_next ~ drtg + drtg_l1 + drtg_l2 +
                    pct_minutes_stay + dbpm_stay +
                    commits + avg_rtg_cmt + max_rtg_cmt +
                    transfers + avg_rtg_tfr + max_rtg_tfr +
                    dbpm,
