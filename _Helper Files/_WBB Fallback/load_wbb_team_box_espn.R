# =============================================================================
# load_wbb_team_box_espn.R
#
# Direct ESPN API replacement for wehoop::load_wbb_team_box(seasons = 2026).
#
# HOW THE ORIGINAL WORKS:
#   load_wbb_team_box() downloads a pre-built parquet from sportsdataverse-data.
#   That parquet is built by espn_wbb_02_team_box_creation.R, which loops over
#   game IDs from the schedule, calls espn_wbb_game_all(game_id), and pulls
...
#   - Produces the exact 59-column schema from wehoop::load_wbb_team_box()
...
parse_wbb_team_box_json <- function(json, game_id) {
...
fetch_wbb_team_box_game <- function(game_id) {
  ...
  tryCatch({
    ...
    parse_wbb_team_box_json(json, game_id)
  }, error = function(e) {
...
# =============================================================================
# MAIN: load_wbb_team_box_espn()
# Drop-in replacement for wehoop::load_wbb_team_box(seasons = 2026)
# =============================================================================

#' Load WBB team box scores directly from ESPN API
#'
#' @description
...
#'
#' Typical workflow:
#'   1. Pull your schedule (which has game_ids) via load_wbb_schedule_espn()
...
#' @param schedule  Optional tibble from load_wbb_schedule_espn() or
#'                  wehoop::load_wbb_schedule(). Used to resolve game_ids from
...
#'
#' @return tibble with same 59-column schema as wehoop::load_wbb_team_box()
load_wbb_team_box_espn <- function(
    game_ids   = NULL,
    schedule   = NULL,
    start_date = NULL,
    end_date   = NULL,
  ...
  if (is.null(game_ids)) {
    ...
    if (length(game_ids) == 0) {
      warning("load_wbb_team_box_espn(): no STATUS_FINAL games in schedule for given date range.")
      return(tibble())
    }
  }
  ...
  if (verbose) message(glue("load_wbb_team_box_espn(): fetching {n} games..."))
  ...
  all_rows <- map_dfr(seq_along(game_ids), function(i) {
    ...
    fetch_wbb_team_box_game(gid)
  })
  
  if (nrow(all_rows) == 0) {
    warning("load_wbb_team_box_espn(): no rows returned. Check ESPN API connectivity.")
    return(tibble())
  }
...
# =============================================================================
# MERGE: combine existing wehoop team_box with fresh ESPN pull
# =============================================================================

#' Merge existing wehoop team box with new ESPN rows
#'
#' New rows replace existing for the same (game_id, team_id).
merge_wbb_team_box <- function(existing, new_rows) {
  ...
  message(glue(
    "merge_wbb_team_box(): {nrow(existing)} existing + ",
...
#'
#' @param game_id  Integer ESPN game ID
diag_wbb_team_box <- function(game_id) {
  url <- glue("{ESPN_WBB_SUMMARY_URL}?event={game_id}")
...
# team_box_new <- load_wbb_team_box_espn(game_ids = new_game_ids)
...
# ── Option C: Merge with existing wehoop baseline ─────────────────────────────
# team_box_base  <- wehoop::load_wbb_team_box(seasons = 2026)   # stale through ~Mar 6
# team_box_full  <- merge_wbb_team_box(team_box_base, team_box_new)
...
#   pull(game_id)
# team_box_today <- load_wbb_team_box_espn(game_ids = yesterday_ids)
# team_box_full  <- merge_wbb_team_box(team_box_full, team_box_today)
# saveRDS(team_box_full, team_box_path)

# ── Diagnostic: check a specific game ─────────────────────────────────────────
# diag_wbb_team_box(401851750)