...
tourn %>% 
  ...
  select(notes_headline, home_abbreviation) %>% 
...
tourney_teams <- tourn %>% 
  ...
  mutate(region = case_when(
    ...
    str_detect(notes_headline, "Regional 3") ~ "Region 3",
    str_detect(notes_headline, "Regional 4") ~ "Region 4",
  )) %>% 
  select(region,
...
tourney_teams %>% 
  with_groups(.groups = region,
              summarise,
              teams = nrow)

# Create Tournament Structure ----------------
bracket.left <- c("Region 1", "Region 4")
bracket.right <- c("Region 2", "Region 3")

tourney_structure <- data.frame(
  region = c(rep("Region 1", 15),
             rep("Region 2", 15),
             rep("Region 3", 15),
             rep("Region 4", 15),
             "Left", "Right", "All"),
