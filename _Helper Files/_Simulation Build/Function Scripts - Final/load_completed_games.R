load_completed_games <- function(season,
  ...
  next_round <- sched %>% 
    ...
    mutate(winner_seed = case_when(
      result > 0 ~ home_seed,
      TRUE ~ away_seed),
      winner_id = case_when(
        result > 0 ~ home_id,
        TRUE ~ away_id)) %>% 
    with_groups(.groups = c(region, winner_to),
                mutate,
                home_next = case_when(
    ...
    left_join(next_round %>% 
                ...
                select(region, game = winner_to,
                       ...
                       away_seed = winner_seed), by = c('region',
                                                    'game')) %>% 
    transmute(region, round_name, round_no,
              away_seed, away_id, away_rest = 0, away_dist, away_score = 0,
              home_seed, home_id, home_rest = 0, home_dist, home_score = 0,
              result = NA_real_,
              winner_to)
