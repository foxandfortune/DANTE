# =============================================================================
# load_mbb_player_box_espn.R
#
# Direct ESPN API replacement for hoopR::load_mbb_player_box(seasons = 2026).
#
...
# Parses json$boxscore$players into the 54-column hoopR player box schema.
#
# USAGE:
...
#' Parse ESPN summary JSON into hoopR-schema player box tibble
#'
...
parse_player_box_json <- function(json, game_id) {
...
fetch_player_box_game <- function(game_id) {
  ...
  tryCatch({
    ...
    parse_player_box_json(json, game_id)
  }, error = function(e) {
...
# =============================================================================
# MAIN: load_mbb_player_box_espn()
# =============================================================================

#' Load MBB player box scores directly from ESPN API
#'
#' @param game_ids   Integer vector of game IDs. If NULL, resolved from schedule.
#' @param schedule   tibble from load_mbb_schedule_espn() or hoopR::load_mbb_schedule().
...
#'
#' @return tibble with same 54-column schema as hoopR::load_mbb_player_box()
load_mbb_player_box_espn <- function(
    game_ids   = NULL,
    schedule   = NULL,
    start_date = NULL,
    end_date   = NULL,
  ...
  if (is.null(game_ids)) {
    ...
    if (length(game_ids) == 0) {
      warning("load_mbb_player_box_espn(): no STATUS_FINAL games in schedule for given date range.")
      return(tibble())
    }
  }
  ...
  if (verbose) message(glue("load_mbb_player_box_espn(): fetching {n} games..."))
  ...
  all_rows <- map_dfr(seq_along(game_ids), function(i) {
    ...
    fetch_player_box_game(gid)
  })
  
  if (nrow(all_rows) == 0) {
    warning("load_mbb_player_box_espn(): no rows returned.")
    return(tibble())
  }
...
#' Merge existing hoopR player box with new ESPN rows
#' New game_ids fully replace existing (all players for that game replaced).
merge_mbb_player_box <- function(existing, new_rows) {
  ...
  message(glue(
    "merge_mbb_player_box(): {n_distinct(existing$game_id)} existing games + ",
