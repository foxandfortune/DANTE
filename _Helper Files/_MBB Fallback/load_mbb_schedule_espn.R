# =============================================================================
# load_mbb_schedule_espn.R  (v2 — full column parity with hoopR)
#
# Replicates hoopR::load_mbb_schedule(seasons = 2026) by hitting ESPN's
...
HOOPR_RAW_BASE  <- "https://raw.githubusercontent.com/sportsdataverse/hoopR-mbb-raw/main/mbb/json/final"
...
parse_scoreboard_json <- function(json) {
  ...
  map_dfr(seq_along(events), function(i) {
    ...
    game_json     <- is_final
    game_json_url <- if (is_final) glue("{HOOPR_RAW_BASE}/{game_id}.json") else NA_character_
    PBP        <- is_final && isTRUE(play_by_play_avail)
    team_box   <- is_final
    player_box <- is_final
    
    # ── Assemble row — column order matches hoopR::load_mbb_schedule() ────────
    tibble(
      id                        = game_id,
      uid                       = uid,
      date                      = date_raw,
...
fetch_scoreboard_date <- function(date_str, groups = c(50, 100)) {
  all_rows <- map_dfr(groups, function(g) {
    url <- glue("{ESPN_SCOREBOARD}?groups={g}&limit=500&dates={date_str}")
    tryCatch({
...
# =============================================================================
# MAIN: load_mbb_schedule_espn()
# =============================================================================

#' Load MBB schedule directly from ESPN API, matching hoopR::load_mbb_schedule()
#'
...
#'
#' @return tibble with same 86-column schema as hoopR::load_mbb_schedule()
load_mbb_schedule_espn <- function(
    season     = 2026,
    start_date = NULL,
    end_date   = NULL,
    groups     = c(50, 100),
  ...
  end_date   <- if (is.null(end_date))   min(Sys.Date(), as.Date(paste0(season, "-04-10"))) else as.Date(end_date)
  
  dates        <- seq(start_date, end_date, by = "day")
  date_strings <- format(dates, "%Y%m%d")
  
  if (verbose) message(glue(
    "load_mbb_schedule_espn(): {length(date_strings)} dates ",
  ...
  all_rows <- map_dfr(seq_along(date_strings), function(i) {
    ...
    fetch_scoreboard_date(date_strings[i], groups = groups)
  })
  
  if (nrow(all_rows) == 0) {
    warning("load_mbb_schedule_espn(): no games returned.")
    return(tibble())
  }
...
#' Combine hoopR schedule baseline with fresh ESPN pull
#'
...
merge_mbb_schedule <- function(existing, new_rows) {
  ...
  message(glue(
    "merge_mbb_schedule(): {nrow(existing)} existing + ",
...
diag_scoreboard <- function(date_str = format(Sys.Date() - 1, "%Y%m%d"), group = 50) {
  url <- glue("{ESPN_SCOREBOARD}?groups={group}&limit=500&dates={date_str}")
