...
# load_mbb_pbp_espn.R
#
# Direct ESPN API replacement for hoopR::load_mbb_pbp(seasons = 2026).
#
...
#
# USAGE (mirrors load_mbb_team_box_espn.R):
...
parse_pbp_json <- function(json, game_id) {
...
fetch_pbp_game <- function(game_id) {
...
# =============================================================================
# MAIN: load_mbb_pbp_espn()
# =============================================================================

#' Load MBB play-by-play directly from ESPN API
#'
...
#'
#' @return tibble with same 57-column schema as hoopR::load_mbb_pbp()
load_mbb_pbp_espn <- function(
    game_ids   = NULL,
    schedule   = NULL,
    start_date = NULL,
    end_date   = NULL,
  ...
  if (is.null(game_ids)) {
    ...
    if (length(game_ids) == 0) {
      warning("load_mbb_pbp_espn(): no STATUS_FINAL games in schedule for given date range.")
      return(tibble())
    }
  }
  ...
  if (verbose) message(glue("load_mbb_pbp_espn(): fetching PBP for {n} games..."))
  ...
  if (nrow(all_rows) == 0) {
    warning("load_mbb_pbp_espn(): no rows returned.")
    return(tibble())
  }
...
#' Merge existing hoopR PBP with new ESPN rows
#' New game_ids replace existing (full game replacement, not play-level).
merge_mbb_pbp <- function(existing, new_rows) {
  ...
  message(glue(
    "merge_mbb_pbp(): {n_distinct(existing$game_id)} existing games + ",
...
# ── Same pattern as load_mbb_team_box_espn: ───────────────────────────────────
#
...
#
# pbp_new <- load_mbb_pbp_espn(
#   schedule   = mbb_schedule_full,   # already built when you ran the schedule script
#   start_date = "2026-03-07"
# )
#
# mbb_pbp_full <- bind_rows(mbb_pbp_base, pbp_new)
