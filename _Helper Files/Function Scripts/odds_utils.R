double_odds <- function(g) {
  g1 <- g %>%
    select(sim, round_name, round_no, region,
           away_id, home_id, away_score, result) %>%
    rename(team_id = away_id, team_score = away_score, opp_id = home_id) %>%
    mutate(result = -1 * result)
  g2 <- g %>%
    select(sim, round_name, round_no, region,
           away_id, home_id, home_score, result) %>%
    rename(team_id = home_id, team_score = home_score, opp_id = away_id)
  g <- bind_rows(g1, g2) %>%
...
saveRDS(double_odds, 'Simulation Backup/Functions/double_odds.rds')


get_an_odds <- function(df){
  odds <- data.frame()
  ...
  for(i in 1:length(df$game_date)){
    ...
    URL = case_when(
      lubridate::day(day) < 10 & lubridate::month(day) < 10 ~
        paste0('https://api.actionnetwork.com/web/v1/scoreboard/ncaab?division=D1&date=',
      ...
      for(j in 1:length(matches$id)){
        temp = if(!is.null(matches$odds[[j]])){
          data.frame(matches$odds[[j]]) %>% 
            ...
            filter(type == "game", book_name == "Consensus") %>% 
