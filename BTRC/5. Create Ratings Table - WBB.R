...
team_logos <- wehoop::espn_wbb_teams(year = 2025) %>% 
  select(team_id, logo) %>% 
...
teams <- teams %>% 
  ...
  mutate(logo = case_when(
    ...
    conference_short_name = case_when(
      is.na(conference_short_name) & team == 'West Georgia' ~ 'ASUN',
      is.na(conference_short_name) & team %in% c('Lindenwood',
                                                 'Southern Indiana') ~ 'OVC',
      is.na(conference_short_name) & team == 'Mercyhurst' ~ 'NEC',
...
results <- wehoop::espn_wbb_standings(2025)
...
scoreboard <- wehoop::load_wbb_schedule(2025)
