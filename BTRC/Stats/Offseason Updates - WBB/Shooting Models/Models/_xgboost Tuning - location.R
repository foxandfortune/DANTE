...
shots_model <- all_shots %>% 
  filter(shooting_play == TRUE, type_id != "540", !is.na(pos), !is.na(shot_dist)) %>% 
...
grid <- dials::grid_latin_hypercube(
  ...
  dials::learn_rate(range = c(-1.45, -0.35), trans = scales::log10_trans()),
...
grid <- dials::grid_latin_hypercube(
  ...
  dials::tree_depth(range = c(3L, 8L)), # max_depth
  # to force learn_rate to not be crazy small like dials defaults to
  dials::learn_rate(range = c(-1.5, -1), trans = scales::log10_trans()), # eta (10 ^ p)
  dials::loss_reduction(),
  sample_size = dials::sample_prop(),
  size = grid_size
) %>%
...
saveRDS(best_model, "Models/xfg_xgboost.rds")

saveRDS(xfg_model, glue::glue("Models/xfg {cur_yr}.rds"))
